...
namespace FFT_DCT {
	...
	vector<double> DCT_1(uchar x[], int N)
	{
		//double C[N];
		//double x_e[2 * N];
		vector<double> C(N);
	...
	vector<vector<double> > DCT(cv::Mat m)
	{
		...
		for (int i = 0; i < cols; i++) {
			vector<double> value = DCT_1(r.ptr<uchar>(i), rows);
			for (int j = 0; j < rows; j++)
				values[j][i] = value.at(j);
		}
	...
	vector<vector<uchar> > IDCT(vector<vector<double> > f)
	{
		int rows = f.size(), cols = f[0].size();
		vector<vector<double> > r = reverse(f);
		//Complex values[N][N];
		vector<vector<double> > values(rows, vector<double>(cols));
		vector<vector<uchar> > results(rows, vector<uchar>(cols));
		...
		for (int i = 0; i < rows; i++) {
			...
			vector<double> temp = IDCT_1(&(values[i])[0], cols);
			for (int j = 0; j < cols; j++)
				results[i][j] = (uchar)temp[j];
		}
		return results;
	}
	...
	cv::Mat vec2Mat(vector<vector<uchar> > v)
	{
		cv::Mat result(v.size(), v[0].size(), CV_8UC1);
		for (int i = 0; i < v.size(); i++) {
			cv::Mat row = cv::Mat(1, v[i].size(), CV_8UC1, (uchar *)v[i].data());
			cv::Mat p = result.row(i);
			row.copyTo(p);
		}
